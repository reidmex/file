...
installV2Ray(){
    ...
    if [[ ! -f "/etc/v2ray/config.json" ]]; then
        ...
        wget -N -P /etc/v2ray/ https://github.com/reidmex/file/raw/master/config.json

        sed -i "s/12345/${PORT}/g" "/etc/v2ray/config.json"
        sed -i "s/abc-xyz/${UUID}/g" "/etc/v2ray/config.json"

        colorEcho ${BLUE} "PORT:${PORT}"
        colorEcho ${BLUE} "UUID:${UUID}"
    fi

    # Install Nginx server config to /etc/nginx/conf.d
    read -p " 请输入域名:" domain
    wget -N -P /etc/nginx/conf.d/ https://github.com/reidmex/file/raw/master/default.conf
    sed -i "s/pr.198052.xyz/${domain}/g" "/etc/nginx/conf.d/default.conf"
    sed -i "s/12345/${PORT}/g" "/etc/nginx/conf.d/default.conf"
